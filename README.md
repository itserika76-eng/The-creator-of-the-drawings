...
> ⚠️ В Linux/dev-контейнере COM API КОМПАС обычно недоступен, поэтому используется fallback-экспорт. На Windows с установленным КОМПАС и `pywin32` нужно донастроить вызовы API под вашу версию.
...
- `<project>.cdw` и `<project>.spw` — экспорт через слой КОМПАС API / fallback.
