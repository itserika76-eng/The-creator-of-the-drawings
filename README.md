...
### Новая стратегия плавного построения и авто-исправления
- Рисование выполняется пакетами по сегментам, с плавным перемещением курсора.
- После каждого пакета берется снимок рабочего поля и сравнивается с оригиналом (edge-map).
- Если пакет ухудшает качество совпадения, программа откатывает действия (`Ctrl+Z`) и пробует продолжать дальше.
- Это снижает вероятность «дичи» из случайных линий и добавляет самокоррекцию прямо в процессе построения.
